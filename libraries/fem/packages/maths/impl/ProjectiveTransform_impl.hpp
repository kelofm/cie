...
void ProjectiveTransformDerivative<TValue,Dimension>::evaluate(
    ConstSpan in,
    Span out,
    BufferSpan) const {
        ...
        const TValue lastRowProduct = std::inner_product(
...
template <concepts::Numeric TValue, unsigned Dimension>
constexpr unsigned ProjectiveTransformDerivative<TValue,Dimension>::bufferSize() noexcept {
    return 0u;
}
...
template <concepts::Numeric TValue, unsigned Dimension>
constexpr unsigned ProjectiveTransform<TValue,Dimension>::bufferSize() noexcept {
    return 0u;
}


template <concepts::Numeric TValue, unsigned Dimension>
void ProjectiveTransform<TValue,Dimension>::evaluate(
    ConstSpan in,
    Span out,
    BufferSpan) const {
        assert(Dimension == in.size());
        assert(Dimension == out.size());
