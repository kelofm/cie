...
void OrthogonalScaleTransformDerivative<TValue,Dimension>::evaluate(
    ConstSpan,
    Span out,
...
template <concepts::Numeric TValue, unsigned Dimension>
constexpr unsigned OrthogonalScaleTransformDerivative<TValue,Dimension>::bufferSize() noexcept {
    return 0u;
}
...
OrthogonalScaleTransform<TValue,Dimension>::OrthogonalScaleTransform(
    ...
    [[maybe_unused]] TPointIt itTransformedEnd) {
        assert(std::distance(itTransformedBegin, itTransformedEnd) == 1);
...
template <concepts::Numeric TValue, unsigned Dimension>
void OrthogonalScaleTransform<TValue,Dimension>::evaluate(
    ConstSpan in,
    Span out,
    BufferSpan) const {
        assert(in.size() == Dimension);
        std::transform(
            in.begin(),
            in.end(),
            this->_scales.begin(),
