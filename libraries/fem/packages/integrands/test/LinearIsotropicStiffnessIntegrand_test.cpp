...
CIE_TEST_CASE("LinearIsotropicStiffnessIntegrand - tracked allocator", "[integrands]") {
...

    {
        ...
        const Ansatz producerAnsatzSpace(Ansatz::AnsatzSet({
            Basis(Basis::Coefficients ({ 0.5,  0.5}, producerAllocator)),
            Basis(Basis::Coefficients ({ 0.5, -0.5}, producerAllocator))},
            producerAllocator));

        // Compute the derivatives of the ansatz space.
        const Ansatz::Derivative producerAnsatzDerivatives = producerAnsatzSpace.makeDerivative();
        ...
        cie::io::BinarySerializer::serialize(
            stream,
            producerAnsatzDerivatives);

        // Deserialize to consumers.
