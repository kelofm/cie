...
template <
    ...
    class TGraphData,
    unsigned Dimension>
void Assembler::addGraph(
    Ref<const Graph<TVertexData,TEdgeData,TGraphData>> rGraph,
    Ref<const AnsatzMap<Dimension>> rAnsatzMap,
    std::size_t dofsPerCell) {
        return this->addGraph<TVertexData>(
            rGraph,
            [&rGraph] (VertexID id) -> Ref<const TVertexData> {
                return rGraph.find(id).value().data();},
            rAnsatzMap,
            dofsPerCell);
}
...
void Assembler::addGraph(
    ...
    std::size_t dofsPerCell) {
        CIE_BEGIN_EXCEPTION_TRACING
            ...
            std::queue<Ptr<const typename TGraph::Vertex>> visitQueue {{&rGraph.vertices().front()}};
            tsl::robin_set<VertexID> visited;
            ...
            while (!visitQueue.empty()) {
                ...
                Ref<const typename TGraph::Vertex> rVertex = *visitQueue.front();
                ...
                for (const auto edgeID : rVertex.edges()) {
                    Ref<const typename TGraph::Edge> rEdge = rGraph.find(edgeID).value();

                    Ptr<const typename TGraph::Vertex> pSource, pTarget;
                    ...
                    Ref<const TCell> rSource = rCellGetter(pSource->id());
                    Ref<const TCell> rTarget = rCellGetter(pTarget->id());
                    ...
                    const OrientedBoundary<Dimension> sourceBoundary(rSource.axes(), rEdge.data().boundary());
                    const OrientedBoundary<Dimension> targetBoundary(rTarget.axes(), rEdge.data().boundary());
