...
ParallelFor<TIndex,TStorage>::operator()(
        ...
        CIE_BEGIN_EXCEPTION_TRACING
            this->executeImpl(
                DynamicIndexPartitionFactory(
                    {indexMin, indexSentinel, stepSize},
                    _pool.size()),
                rFunction,
                true);
            return *this;
...
ParallelFor<TIndex,TStorage>::operator()(
    ...
    Ref<const TFunction> rFunction) {
        CIE_BEGIN_EXCEPTION_TRACING
            this->executeImpl(
                DynamicIndexPartitionFactory(
                    {0, static_cast<std::size_t>(std::distance(itBegin, itEnd)), 1},
                    _pool.size()),
                itBegin,
                rFunction);
            return *this;
...
template <concepts::Integer TIndex, class TStorage>
template <class TFunction>
void ParallelFor<TIndex,TStorage>::executeImpl(
                ...
                for (TIndex iPartition=0; iPartition<static_cast<TIndex>(partitionCount); ++iPartition) {
                    ...
                    _pool.queueTLSJob(
                        [partition, &rFunction](auto&... rArgs) -> void {
                            if constexpr (cie::concepts::CallableWith<TFunction,TIndex,decltype(rArgs)&...>) {
                                for (TIndex i=partition.begin; i<static_cast<TIndex>(partition.end); i+=partition.step) {
                                    rFunction(i, rArgs...);
                                }
                            } else if constexpr (cie::concepts::CallableWith<TFunction,TIndex,TIndex,decltype(rArgs)&...>) {
                                rFunction(partition.begin, partition.end, rArgs...);
                            } else
                                static_assert(std::is_same_v<TFunction,void>, "unsupported function signature");
                        }
                    ); // queueJob
